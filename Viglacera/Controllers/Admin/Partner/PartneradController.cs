...
namespace Viglacera.Controllers.Admin.Partner
{
    public class PartneradController : Controller
    {
        //
        // GET: /Partnerad/
        ...
        public ActionResult Index(int? page, string id, FormCollection collection)
        {
            ...
            {
                var listPartner = db.tblPartners.ToList();
                        ...
                        if (key.StartsWith("chk_"))
                        {
                            ...
                            if (checkbox != "false")
                            {
                                if (ClsCheckRole.CheckQuyen(15, 3, int.Parse(Request.Cookies["Username"].Values["UserID"])) == true)
                                {
                                    ...
                                    tblPartner tblParner = db.tblPartners.Find(ids);
                                    db.tblPartners.Remove(tblParner);
                                    db.SaveChanges();
                ...
                return View(listPartner.ToPagedList(pageNumber, pageSize));
            }
            else
            {
                return Redirect("/Users/Erro");
        ...
        public ActionResult UpdatePartner(string id, string Active, string Ord)
        {
            if (ClsCheckRole.CheckQuyen(15, 2, int.Parse(Request.Cookies["Username"].Values["UserID"])) == true)
            {

                int ids = int.Parse(id);
                var tblPartner = db.tblPartners.Find(ids);
                tblPartner.Active = bool.Parse(Active);
                tblPartner.Ord = int.Parse(Ord);
                db.SaveChanges();
        ...
        public ActionResult Create()
        {
            ...
            {
                var pro = db.tblPartners.OrderByDescending(p => p.Ord).ToList();
                if (pro.Count > 0)
        ...
        [HttpPost]
        public ActionResult Create(tblPartner tblpartner, FormCollection collection)
        {

            db.tblPartners.Add(tblpartner);
            db.SaveChanges();
            #region[Updatehistory]
            Updatehistoty.UpdateHistory("Add tblPartner", Request.Cookies["Username"].Values["FullName"].ToString(), Request.Cookies["Username"].Values["UserID"].ToString());
            #endregion
            if (collection["btnSave"] != null)
            {
                ...
                return Redirect("/partnerad/Index");
            }
            if (collection["btnSaveCreate"] != null)
            {
                ...
                return Redirect("/partnerad/Create");
            }
        ...
        [HttpPost]
        public ActionResult Edit(tblPartner tblpartner, int id, FormCollection collection)
        {
            if (ModelState.IsValid)
            {
                ...
                db.SaveChanges();
                #region[Updatehistory]
                Updatehistoty.UpdateHistory("Edit tblpartner", Request.Cookies["Username"].Values["FullName"].ToString(), Request.Cookies["Username"].Values["UserID"].ToString());
                #endregion
                if (collection["btnSave"] != null)
                {
